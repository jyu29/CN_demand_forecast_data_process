...
def check_d_sku(df):
    ...
    """
    sku_last = df \
        .filter(F.col('sku_date_end') == '2999-12-31 23:59:59') \
        .groupby('sku_idr_sku') \
        .agg((F.count(F.col('sku_num_sku_r3'))).alias('count_nb'))
    sku_duplicate = sku_last.filter(sku_last.count_nb > 1)
    print(f'Sku duplicate : {sku_duplicate.count()}')

    sku_wrong_date = df.filter(F.col('sku_date_begin') > F.col('sku_date_end'))
    print(f'Sku date_begin > date_end : {sku_wrong_date.count()}')

    sku_max_tech_date = df.agg(F.max('rs_technical_date')).collect()[0][0]
...
def check_d_business_unit(df):
    ...
    """
    but_count = df \
        .filter(F.col('but_num_typ_but').isin({'7', '48', '50'})) \
        .groupby('but_idr_business_unit') \
        .agg((F.count(F.col('but_idr_business_unit'))).alias('count_nb'))
